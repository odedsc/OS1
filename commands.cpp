...
//		sort(sort(job_stack.begin(), job_stack.end(), ID_sort));
...

	{
		...
		if ((num_arg!=0) || (num_arg!=1)) illegal_cmd = true;
...

	{
		...
		if (ExeComp(lineSize) == -1){ // check if the external command is complicated

			if (BgCmd(lineSize,jobs)==-1){ // check if the external command is not complicated needs to run in the background
...
void ExeExternal(char *args[MAX_ARG], const char* cmdString)
...
//	char ExtCmd[MAX_LINE_SIZE+2];
	...
	const char *args[MAX_ARG]={"/bin/csh", "-f", "-c", lineSize, NULL};
